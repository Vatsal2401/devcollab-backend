...
export class PlansService {
  ...
  private generatePlanMd(plan: PlanEntity, username: string): string {
    const repos = (() => { try { return JSON.parse(plan.reposAffected) as string[]; } catch { return []; } })();
    const criteria = (() => { try { return JSON.parse(plan.acceptanceCriteria) as { text: string; checked: boolean }[]; } catch { return []; } })();
    const testCases = (() => { try { return JSON.parse(plan.qaTestCases) as { text: string; checked: boolean }[]; } catch { return []; } })();
    const repoList = repos.length > 0 ? repos.map((r) => `- ${r}`).join('\n') : '_No repos specified._';
    const criteriaList = criteria.length > 0 ? criteria.map((c) => `- [${c.checked ? 'x' : ' '}] ${c.text}`).join('\n') : '_No acceptance criteria defined._';
    const testCaseList = testCases.length > 0 ? testCases.map((t) => `- [${t.checked ? 'x' : ' '}] ${t.text}`).join('\n') : '_No QA test cases defined._';

    return `# ${plan.id}: ${plan.title}

> **Type:** ${plan.type} | **Priority:** ${plan.priority} | **Status:** ${plan.status} | **Estimated:** ${plan.estimatedHours || '?'}h
> **Branch:** \`${plan.branch || 'TBD'}\` | **Repos:** ${repos.join(', ') || 'None'}
> **Author:** ${username} | **Assigned:** ${plan.assignedTo || 'Unassigned'}
...
${plan.background || 'No background provided.'}
...
${plan.implementationSteps || 'Steps to be defined.'}

---

## Claude Instructions

> These are specific instructions for Claude Code AI assistant executing this plan.

${plan.claudeInstructions || 'Follow standard coding patterns for this codebase.'}
...
**Comments:** ${plan.reviewComments || '_No review comments yet._'}

---

## QA Notes

${plan.qaNotes || '_No QA notes yet._'}

---
*Generated by DevCollab | ${new Date().toISOString()}*
`;
  }
  ...
  async getSessionInfo(id: string): Promise<{
    ...
    const plan = await this.findById(id);
    const tmuxSession = plan.id.toLowerCase().replace('plan-', 'plan');
    const worktreePath = `${process.env.WORKSPACES_DIR || '/workspaces'}/active`;
    const serverHost = process.env.SERVER_HOST || 'devcollab.autoreels.in';
    const serverUser = process.env.SERVER_USER || 'vatsal2401';
    ...
    return {
      ...
      worktreePath,
      tmuxSession,
      sshCommand: `ssh ${serverUser}@${serverHost} -t "tmux attach-session -t ${tmuxSession} || tmux new-session -t ${tmuxSession}"`,
      status: plan.status,
    };
  }
}
